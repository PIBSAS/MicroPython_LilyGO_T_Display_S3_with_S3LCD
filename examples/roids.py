...
def main():
    ...
    def update_missiles():
        ...
        for missile in list(missiles):                          # for each missile
            missile.draw(s3lcd.BLACK)                          # erase old missile
            if missile.counter > 0:                             # if counter > 0 missile is active
                ...
                for roid in list(roids):                        # for each roid
                    if missile.collision(roid):                 # check if missile collides with roid
                        roid.draw(s3lcd.BLACK)                 # erase the roid
                ...
                if missile.counter > 0:                         # if the missile has life left
                    missile.draw(s3lcd.WHITE)                  # draw missile
    ...
    def update_roids():
        ...
        for roid in roids:                  # for each roid, erase, move then draw
            roid.draw(s3lcd.BLACK)
            roid.move()
            roid.draw(s3lcd.WHITE)

            if roid.collision(ship):        # check for roid/ship collision
                ship.draw(s3lcd.BLACK)     # erase ship
    ...
    def explode_ship():
        ...
        ship.counter += 1
        if ship.counter % 2 == 0:
            tft.polygon(explosion_poly, ship.x, ship.y, s3lcd.BLACK, 255,  0.785398)
            tft.polygon(explosion_poly, ship.x, ship.y, s3lcd.WHITE)
        else:
            tft.polygon(explosion_poly, ship.x, ship.y, s3lcd.WHITE, 255, 0.785398)
            tft.polygon(explosion_poly, ship.x, ship.y, s3lcd.BLACK)
    ...
    try:
        ...
        frame_time = 30 # target frame rate delay
